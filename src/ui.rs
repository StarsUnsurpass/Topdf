...
impl App {
    pub fn new() -> (Self, Task<Message>) {
        ...
        for path in system_fonts {
            if let Ok(bytes) = std::fs::read(path) {
                ...
                if let Ok(font) = converter::prepare_font(bytes_arc) {
                    println!("Successfully loaded system font: {}", path);
        ...
        let font = selected_font.unwrap_or_else(|| {
             println!("Loading embedded fallback font (Roboto).");
        ...
        match message {
            ...
            Message::ConversionFinished(index, result) => {
                ...
                if let Some(file) = self.files.get_mut(index) {
                    match result {
                        Ok(_) => file.status = ConversionStatus::Success,
                        Err(e) => file.status = ConversionStatus::Error(e),
                    }
                }
    ...
    pub fn view(&self) -> Element<'_, Message> {
        ...
        container(column![header, main_content].spacing(40))
            .width(Length::Fill)
            .height(Length::Fill)
            .padding(40)
            .style(|_theme| container::Style {
